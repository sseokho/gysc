...
function se1Box1_Swiper(){
  var autoPlayDelay = 3000;
	...
	var options = {
    	...
    	navigation: {
			nextEl: '.box1_btn .swiper-button-next',
			prevEl: '.box1_btn .swiper-button-prev',
    	},
  	};
	...
	for(let i=0; i<slidersCount; i++){
		$('.box1_btn .swiper-progress-bar .progress-sections').append('<span></span>');
	}
	
	function initProgressBar(){
		let calcProgress = (slidersCount-1) * (autoPlayDelay + mySwiper.params.speed);
        calcProgress += autoPlayDelay;
		$('.box1_btn .swiper-progress-bar .progress').animate({
			width: '100%'
		}, calcProgress, 'linear');
	}
	...
	mySwiper.on('slideChange', function () {
		let progress = $('.box1_btn .swiper-progress-bar .progress');
		
		$('.box1_btn .swiper-counter .current').html(this.activeIndex + 1);
